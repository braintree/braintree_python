...
class ApplePayGateway(unittest.TestCase):
    ...
    def test_register_domain_gets_a_validation_error_when_attempting_to_register_no_domains(self):
        ...
        self.asserEqual(result["errors"]["apple_pay"][0]["message"], "Domain name is required.")
    ...
    def test_registered_domains_returns_stubbed_registered_domains(self):
        ...
        self.assertTrue(result.is_success)
        self.assertEqual(len(result["apple_pay_options"]["domains"]), 1)
        self.assertEqual(result["apple_pay_options"]["domains"][0], "www.example.com")
