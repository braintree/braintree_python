...
class TestSearch(unittest.TestCase):
    ...
    def test_in_list_with_active(self):
        expected = ["active"]
        self.assertEquals("status", Search.status.name)
        self.assertEquals(expected, Search.status.in_list(["active"]).to_param())

    def test_in_list_with_canceled(self):
        expected = ["canceled"]
        self.assertEquals("status", Search.status.name)
        self.assertEquals(expected, Search.status.in_list(["canceled"]).to_param())
